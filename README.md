...
- **Efficient triggers** — the blueprint's moisture-max stop uses a `numeric_state` trigger tied directly to the threshold entity, so it only fires when moisture actually crosses that line — not on every routine sensor update. It also requires moisture to stay above the threshold for 30 seconds before firing, so brief flickers right at the boundary don't cause repeated start/stop flapping.
